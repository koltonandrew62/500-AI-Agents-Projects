/**
 * OpenRouter implementation of the `LLMProvider` interface (types.ts).
 *
 * Talks to `https://openrouter.ai/api/v1/chat/completions` via the global
 * `fetch`. One `OpenRouterProvider` instance is bound to a single model --
 * `ModelRouter` owns picking *which* model to use and falling back between
 * them.
 *
 * Supports:
 *  - SSE token streaming, including OpenAI-style `tool_calls` deltas that
 *    arrive fragmented by index and must be accumulated before a caller can
 *    be handed a complete, parseable tool call.
 *  - Vision turns via OpenAI-style multimodal `content` arrays (`image_url`
 *    parts holding a base64 data URI).
...
// Informational attribution headers, not credentials.
const DEFAULT_HTTP_REFERER = 'https://github.com/jarvis-assistant';
const DEFAULT_X_TITLE = 'J.A.R.V.I.S.';
 ...
 *
 * The function `name` typically arrives whole on the first fragment for a
 * given index; `arguments` arrives as successive partial JSON-string
 * fragments that must be concatenated before parsing. There is no explicit
 * "this call is done" marker per-call -- completeness is only known once the
 * stream reports `finish_reason` (or ends).
 */
...
export class OpenRouterProvider implements LLMProvider {
  ...
  constructor(model: string, opts: OpenRouterOptions = {}) {
    this.model = model;
    this.apiKey = opts.apiKey;
    this.timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
    this.httpReferer = opts.httpReferer ?? DEFAULT_HTTP_REFERER;
    this.xTitle = opts.xTitle ?? DEFAULT_X_TITLE;
  }
